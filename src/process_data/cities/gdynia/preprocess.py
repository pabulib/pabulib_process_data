...
@dataclass
class Preprocess(GetVotesExcel):
    ...
    def transform_df(self, df):
        ...
        result["citywide_vote"] = citywide_df.stack().reset_index().groupby('level_0')[
            'level_1'].transform(lambda x: ','.join(x))
        result["district_vote"] = district_df.stack().reset_index().groupby('level_0')[
            'level_1'].transform(lambda x: ','.join(x))
